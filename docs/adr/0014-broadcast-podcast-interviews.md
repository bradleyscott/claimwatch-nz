...
**Comparative pricing for the residual lane** (2026 published rates, verified): at the residual volume (~4–6 audio-hours/day ≈ 300 hours through the election window) — self-hosted Whisper ≈ **$0** (electricity only); AssemblyAI Universal-2 ≈ **$45** total; gpt-4o-mini-transcribe ≈ **$54**; Google Chirp 2 dynamic-batch ≈ **$72** (24-hour turnaround); Deepgram Nova-3 ≈ **$78**; gpt-4o-transcribe ≈ **$108**. So the paid-services ceiling for the whole election window is **$45–108** — affordable, but self-hosting is free, keeps audio on our hardware (no egress), and adds speaker diarization via pyannote at no extra charge (AssemblyAI charges extra for its diarization tier; per-hour rates above are transcription-only). Paid services remain the right choice if homelab GPU capacity is unavailable or a burst volume (e.g. debate week) exceeds it: the integration is a one-hour change either way.
...
**Bottom line:** no published NZ-specific WER exists; inferential evidence suggests Whisper large-v3-Turbo lands around 8–15% WER on NZ broadcast interview audio (roughly at the top edge of human transcription performance on clean audio, 1.5–2× human error on noisy multi-speaker audio) — adequate for claim detection with the existing guardrails, and the pre-ship 30-segment local benchmark turns the inference into a measured number.
