...
- **Graphile Worker for job scheduling and queueing** — ingestion cadence, scoring-run triggers, re-probes, backfills, ad-hoc reprocesses. Postgres-backed (jobs are rows, `LISTEN/NOTIFY` wake, `SKIP LOCKED` claiming): the schedule survives restarts, run history feeds ADR-0012's job-health metrics and silence-detection, and Node jobs run in worker containers without a SQL-only middleman. This replaces both cron-on-the-host and a job framework: the scheduler is the database. Design: `docs/design/STORE.md` §2.3.
...
- Postgres is one database; pgvector is an extension install on any hosted Postgres. Ops surface: one database, no new services. Job scheduling/queueing: Graphile Worker on the same database (design: `docs/design/STORE.md` §2.3).

**What is deliberately NOT adopted:** a separate vector database (Qdrant/Milvus/weaviate — unnecessary at our scale, adds a service), an orchestration engine (Temporal/Airflow/Prefect — Graphile Worker covers the slice; orchestration is revisitable at operations-hardening), and Redis (a second stateful service; Graphile Worker stores job state in the existing Postgres).
...
- **Celery/BullMQ + Redis for jobs.** Rejected: Redis is a second stateful service to run and back up. Graphile Worker stores job state in the Postgres we already run; the escalation path (a dedicated queue service) stays open at operations-hardening.
...
- **Graphile Worker's trade-off is understood**: its crontab fires from worker processes — if no worker is up, schedules don't fire (crontab backfill re-creates missed jobs on restart, and silence-detection covers the gap). The alternative (pg_cron as DB-side trigger) was rejected as a SQL-only middleman that still needed a hand-rolled queue to reach Node.
