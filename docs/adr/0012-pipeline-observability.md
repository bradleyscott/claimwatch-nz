...
- **Grafana Cloud (free tier)** — the operations surface the pipeline needs that PostHog doesn't provide. Grafana's unique place vs PostHog comes down to **metrics-shaped jobs that event analytics handles awkwardly**: (1) **scheduled-job monitoring** — nightly re-verification, reprocessing backfills, and health re-probes are cron work whose primary failure mode is *silence* (a job that never ran emits no event for PostHog to count); Grafana's synthetic monitoring/dead-man's-switch alerts fire on the *absence* of a heartbeat, which is the one signal PostHog's event model structurally cannot produce; (2) **long-retention, low-cardinality metrics** — per-source rates (Tier-2 fallback, unattributed rate, fetch success) as Prometheus-style time series with months of retention, queryable as "last week vs the same week last month," versus PostHog's event tables with 30-day free retention; (3) **alert rules as first-class objects** — threshold/no-data/composite conditions (the committed 80%-cost alert, source-down >4h, reconciliation drift) managed as monitorable configuration rather than ad-hoc queries. PostHog has alerts, but they're analytics-threshold alerts on its own event products — Grafana's are infrastructure-shaped: data-absence detection, multi-signal composition, and independent of any single product's data model.
