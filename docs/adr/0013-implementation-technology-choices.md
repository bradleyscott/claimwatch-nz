...
- **Drizzle vs Prisma vs raw pg for the store package.** Chose **raw `pg`/`postgres` driver + hand-written SQL + a thin query layer**: the store's queries ARE the project's logic (funnel views, fingerprint matching, retrieval), and an ORM's abstraction over SQL is exactly the wrong place to lose control; migrations via `dbmate`/`drizzle-kit` style plain SQL files.
