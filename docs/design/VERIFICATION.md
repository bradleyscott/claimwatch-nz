...
| # | Question | Notes |
|---|---|---|
| 1 | Search-provider order — **resolved**: ADR-0014 previously said "Serper primary (ADR-0011 unchanged)", contradicting ADR-0011's provider table (Brave primary + Serper fallback/bulk). ADR-0011 governs provider selection; ADR-0014 re-affirms it — **Brave primary** (T1–T6 authority restriction via Goggles), **Serper fallback/bulk**. Bradley confirmed 10 Sep 2026. Per-query routing nuance (authority-restricted queries → Brave, bulk → Serper) remains available to `search_config` | Resolved — `search_config` may finalise per ADR-0011 |
