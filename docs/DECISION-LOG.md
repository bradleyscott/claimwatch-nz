...
- **ADR-0009 revised same day** per Bradley's review: proposers no longer classify (matrix placement and trust tiers are project analysis done in review — proposer provides free-text narrative only); domain proposals removed entirely (policy domains are claim-derived from the corpus, not proposed — removes the hardest adjudication case). Templates reduced three → two, both now lightweight narrative forms.
